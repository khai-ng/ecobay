...
export interface CartProductItemProps extends ProductProps {
  check: boolean;
  qty: number;
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="max-w-6">
        <input type="checkbox" className="w-full" checked={product.check} onChange={checkChangeEvent}/>
      </div>
      <div className="basis-[50%] flex gap-3">
        ...
        <div className="flex flex-col gap-1">
          <span className="line-clamp-2 text-ellipsis">{props.name}</span>
        </div>
        <div className="flex flex-col gap-1">
      ...
      <div className="flex-1 text-center">₫{props.price}</div>
